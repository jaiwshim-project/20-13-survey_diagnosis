...
function createExecutiveSurvey() {
  const sections = [
    {
  ...
  const paragraphs = [
    ...
    new Paragraph({
      text: '협회의 구조·전략·거버넌스에 관한 진단 설문',
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 }
    }),
    new Paragraph({
      text: '성실한 응답이 정확한 진단의 기초가 됩니다.',
      alignment: AlignmentType.CENTER,
      italics: true,
      spacing: { after: 800 }
    })
...
function createEmployeeSurvey() {
  ...
  const paragraphs = [
    ...
    new Paragraph({
      text: '협회의 실행·병목·조직문화에 관한 진단 설문',
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 }
    }),
    new Paragraph({
      text: '완전히 익명으로 처리되며 솔직한 응답을 부탁드립니다.',
      alignment: AlignmentType.CENTER,
      italics: true,
      spacing: { after: 800 }
    })
...
async function generateDocuments() {
  try {
    // 출력 디렉토리 생성
    const outputDir = 'survey_docx';
    if (!fs.existsSync(outputDir)) {
      fs.mkdirSync(outputDir, { recursive: true });
    }

    // 회장단 본진단
    const execDoc = createExecutiveSurvey();
    const execPath = path.join(outputDir, '회장단본진단.docx');
    const execBuffer = await Packer.toBuffer(execDoc);
    fs.writeFileSync(execPath, execBuffer);
    console.log(`✅ 회장단 본진단 생성 완료: ${execPath}`);

    // 직원 본진단
    const empDoc = createEmployeeSurvey();
    const empPath = path.join(outputDir, '직원본진단.docx');
    const empBuffer = await Packer.toBuffer(empDoc);
    fs.writeFileSync(empPath, empBuffer);
    console.log(`✅ 직원 본진단 생성 완료: ${empPath}`);
